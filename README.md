...
如果你更倾向于用 Docker 运行 InkOS，项目提供了开箱即用的 `Dockerfile` 和 `docker-compose.yml`。

**方式一：docker compose（推荐）**

```bash
# 克隆仓库
git clone https://github.com/mvpdark/inkos-docker.git
cd inkos-docker

# 按需编辑 docker-compose.yml 中的 LLM 环境变量，然后启动
docker compose up -d
```
...
**数据持久化**：`/workspace` 是命名卷 `inkos-workspace`，所有书籍、配置和产物都持久化在这里。

**环境变量**

| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `STUDIO_PORT` | `4567` | Studio Web 服务端口 |
